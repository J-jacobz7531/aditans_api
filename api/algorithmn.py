...
def Backend(request):
	...
	clients = serviceProvider.objects.all().order_by('-id')
	dataset = serviceProvider.objects.values_list('lat', 'lon', 'id')
	...
	for neighbour in neighbours:
		pk = neighbour[2]
		result = serviceProvider.objects.get(id=pk)
		serializer = ProfileSerializer(result, many=False)
